...
class CountryController extends Controller
{
    ...
    {
        if(Auth::check()){
            $country = Country::select('id','country_name','status','country_namear')
    ...
    public function countryadd(Request $request)
    {
        if(Auth::check()){
            ...
            $country->country_namear = $request->input('countrynamear');
            $country->status = '1';
            $country->save();
    ...
    {         
       ...
       $country = Country::where('id',$id)
                ->select('id','country_name','country_namear')
                ->first();
    ...
    { 
        ...
        $country->country_namear = $request->input('countrynameeditar');
        $country->update();
