...
class StaffController extends Controller
{
   ...
   public function viewUserRole()
   {
      if(Auth::check()){
            ...
            return view('user_role_list', ['userroles' => $userrole]);
    ...
    {
        if(Auth::check()){
           $department_data = UserRole::get();
           return view('user_role',compact('department_data'));
        }
        return redirect("/")->withError('You do not have access');
    }
    ...
    public function addUserRole(Request $request)
    {
            ...
            $userRole->name                 = $request->input('userrole');
            $userRole->name_arabic          = $request->input('userrolear');
            $userRole->description          = $request->input('description');
            $userRole->description_arabic   = $request->input('description_ar');
            $userRole->status = '1';
            $userRole->save();
            
            return redirect("user-role-list")->withSuccess('User Role added successfully');
     ...
     public function edituserrole($id)
    {
        if(Auth::check()){
            $userrole = UserRole::where('id', $id)
               ->first();
        return view('user_role_edit', compact('userrole'));
         }
         return redirect("/")->withError('You do not have access');
    }
      ...
      public function updateuserrole(Request $request, $id)
        {
            ...
            return redirect("user-role-list")->withSuccess('updated successfully');
        }
        
        public function blockuserrole(Request $request, $id)
        {
            $userrole = UserRole::find($id);
            $userrole->status = '0';        
            $userrole->update();
            return redirect("user-role-list")->withSuccess('User Role blocked successfully');
        }
    
        public function unblockuserrole(Request $request, $id)
        {
            $userrole = UserRole::find($id);
            $userrole->status = '1';        
            $userrole->update();
...
*/
    ...
    public function permission(Request $request, $id)
    {
        ...
        $staffdetails = Staff::select('staff.*', 'departments.department_name as departmentname', 'designation.designation_name as designationname')
                ...
                ->join('designation', 'designation.id', '=', 'staff.designation_id')
                ->get();
         return view('permission', [ 'menulist' => $menus,'staffdetails' => $staffdetails]);
    }
