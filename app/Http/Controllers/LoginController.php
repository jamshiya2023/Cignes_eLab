    ...
    public function dashboard()
    {
        if(Auth::check()){
           ...
           // $menus=Permission::with('menus')->where('staff_id', $userId)->get();
            //dd($menus);
            $customerCount  = Customer::select('id', Customer::raw('count(*) as total_count'))
            ->orderBy('id', 'desc')
            ->get();
        
            $inoviceCount  = Invoice::select('id', Customer::raw('count(*) as total_count'))
            ->orderBy('id', 'desc')
            ->get();
        
            $expenseCount  = Expense::sum('amount');
        
            $invoicePaidAmount = Invoice::sum('paidamt');
        
            $invoiceDueAmount = Invoice::sum('balanceamt');
            return view('dashboard', ['customerCount' => $customerCount,'expenseCount' => $expenseCount,'customerCount' => $customerCount,'inoviceCount' => $inoviceCount,'invoicePaidAmount'=>$invoicePaidAmount,'invoiceDueAmount' => $invoiceDueAmount]);
        }
