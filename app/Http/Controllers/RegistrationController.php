...
class RegistrationController extends Controller
{
    ...
    {
        if(Auth::check()){
            $master_insurance=MasterInsurance::where("status", "=", 1)->get();
  ...
  public function registrationlist()
          ...
          ->select('customer.*','registration.*','invoice.id as inv_id','invoice.invoice_number')
          ->orderBy("registration.id", "desc")
          ->get();
        //   return view('registrationlist')->with('CustomerDetails', $CustomerDetails); 
          return view('registrationlist', ['CustomerDetails'=> $CustomerDetails, 'regno'=>'', 'date'=>'', 'customer'=>'', 'invoiceno'=>'']); 
    ...
    public function editregistration($id)
            ...
            $existingdocuments = Customerdocuments::leftJoin('legaldocuments', 'legaldocuments.id','=','customerdocuments.documenttype_id')
                    ->join('registration', 'customerdocuments.cust_id','=','registration.cust_id')
                    ->where('registration.id','=', $id)
     ...
     public function searchlist(Request $request)
    {
     ...
     $invoice_number = $request->invoice_number;
		...
		$CustomerDetails = Customer:: join('registration','customer.id','=','registration.cust_id')
									->join('invoice','registration.id','=','invoice.reg_id');
		    if($rno){
			   $CustomerDetails = $CustomerDetails->where('registration.register_number','like',"%$rno%"); 
            }
			...
			if($invoice_number){
			 ...
			 $CustomerDetails = $CustomerDetails->select('registration.id as id','registration.cust_id as cust_id','customer.name as name','customer.phone as phone','registration.age as age','registration.bloodgroup as bloodgroup','registration.add_line_one as add_line_one','registration.registerdate as registerdate','registration.register_number','invoice.invoice_number')
             ->get(); 
   ...
   /* foreach($request->inputtestid as $index => $tests) {
    ...
    function addregistration(Request $request){ 
       ...
       // dd($existcustcount);
        ...
        if($registercount>0){
            $latestregister= Registration::latest('id')->first();
            $regnumber = strtolower(substr($latestregister->register_number, -3));
            $lastregisternumber = $regnumber;
            $incrementregisternumber = ($lastregisternumber+1);
            $val="REG0";
            $registernumber = str_pad($val.$incrementregisternumber, 6, "0", STR_PAD_LEFT);
        } else 
        {
            $registernumber = 'REG0001';
        }
        
        $registration = new Registration;
        $registration->register_number = $registernumber;
       ...
       // dd($registrationlastid);
        ...
        $totl_paid=$bal=0; //dd($dataArray);
        if (!is_null($dataArray) && !empty($dataArray))
        {
            foreach ($dataArray as $data) {
                
                $paymentmethod = PaymentMethod::where("paymentmethod", "=", $data['paymentMethod'])->first();
            ...
            } else 
            {
                ...
                $totl_paid = $totl_paid + $data['paidAmount'];
                $bal = $totalsubtotal - $totl_paid;
                
                $transaction->balanceamount = $bal;
      ...
      $admin = User::where('role', '1')->first();
        ...
        if ($admin) {
            ...
            $notification = new AdminRegistrationNotification($admin_notfy);
            IlluminateNotification::send($admin, $notification);
        }
        //dd($customerpass);
    ...
    function autocompleteregistration(Request $request){
        ...
        ->select('legaldocuments.documenttype','customerdocuments.documentnumber','customerdocuments.documentexpirydate','customerdocuments.documentexpirydate','customerdocuments.documentfilename','customerdocuments.id as docid')
        ->get();
    ...
    public function invoicelist($registrationlastid)
    {
        ...
        if(Auth::check()){
	        ...
	        ->addSelect('invoice.*','customer.name as name','customer.phone as mob','registration.registerdate as date','registration.registertime as time','registration.add_line_one as address','customer.username')
...
/*$itemlist = Invoice::join('registration', 'invoice.reg_id', '=', 'registration.id')
    ...
    ->join('invoicedetails', 'invoice.id', '=', 'invoicedetails.invoice_id')
    ->join('alltests', 'invoicedetails.test_name', '=', 'alltests.id')
    ->when($paymentMethodIsNotZero, function ($query) {
        return $query->join('master_payment_method', 'invoice.paymentmethod', '=', 'master_payment_method.id');
    })
    ->select(
        Invoice::raw("IF($paymentMethodIsNotZero, master_payment_method.paymentmethod, 'Split') AS paymentmethod"),
        'customer.*',
        'invoice.*',
        'alltests.testname AS testname',
        'invoicedetails.test_unitprice AS price',
        'invoicedetails.test_discount AS discount',
        'invoicedetails.test_tax_amount AS taxamount',
        'invoicedetails.test_subtotal AS total'
    )
    ->when($paymentMethodIsNotZero, function ($query) use ($paymentMethodIsNotZero) {
        return $query->addSelect(Invoice::raw($paymentMethodIsNotZero ? 'master_payment_method.paymentmethod' : "'Split'") . ' AS paymentmethod');
    })
    ->where('registration.id', '=', $registrationlastid)
    ->orderBy('invoice.id', 'desc')
    ->get();*/
    ...
    ->selectRaw("IF($paymentMethodIsNotZero, master_payment_method.paymentmethod, 'Split Method') AS paymentmethod")
    ->addSelect('master_payment_method.paymentmethod AS pay_method')
    ->addSelect('customer.*', 'invoice.*', 'alltests.testname AS testname', 'invoicedetails.test_unitprice AS price', 'invoicedetails.test_discount AS discount', 'invoicedetails.test_tax_amount AS taxamount', 'invoicedetails.test_subtotal AS total')
    ->where('registration.id', $registrationlastid)
    ->orderBy('invoice.id', 'desc')
    ->get();
...
//dd($itemlist);
            ...
            //dd($paymentMethodIsNotZero, $itemlist);
	        $paymentmethod = PaymentMethod::where("status", "=", 1)->get();
    ...
    // function customerdetailsearch(Request $request)
    // {
    //     if($request->get('allsearch'))
    //     {
    //         $query = $request->get('allsearch');      
    //         // fetching result from single table starts here
    //         $count = Customer::where('name', 'LIKE', "%{$query}%")
    //                           ->orwhere('phone', 'LIKE', "%{$query}%")
    //                           ->where('status','1')->count();
            
    //         // $count = Customer::join('customerdocuments','customer.id','=','customerdocuments.cust_id')
    //         //         -> where('customer.name', 'LIKE', "%{$query}%")
    //         //         ->orwhere('customer.phone', 'LIKE', "%{$query}%")
    //         //         ->orwhere('customerdocuments.documentnumber', 'LIKE', "%{$query}%")
    //         //         ->where('customer.status','1')->count();
    ...
    function customerdetailsearch(Request $request)
    {
        if($request->get('allsearch'))
        {
            ...
            $data =   Customer::join('customerdocuments','customer.id','=','customerdocuments.cust_id')
                    -> where('customer.name', 'LIKE', "%{$query}%")
                    ->orwhere('customer.phone', 'LIKE', "%{$query}%")
                    ->orwhere('customerdocuments.documentnumber', 'LIKE', "%{$query}%")
            ...
            $output = '<ul class="dropdown-menu" style="display:block; position:relative">';
                foreach($data as $row)
                {
                    $output .= '<li class="custname" style="margin-left:1rem;" id='.$row->cust_id.'>'.$row->name.'-'.$row->phone.'-'.$row->documentnumber.'</li>';
                }
                    $output .= '</ul>';            
            }
  ...
  /*  function customerdetailsearch(Request $request)
    {
        if($request->get('allsearch'))
        {
            ...
            $count1 = Customer::join('customerdocuments','customer.id','=','customerdocuments.cust_id')
                    -> where('customer.name', 'LIKE', "%{$query}%")
                    ->orwhere('customer.phone', 'LIKE', "%{$query}%")
                    ->orwhere('customerdocuments.documentnumber', 'LIKE', "%{$query}%")
                    ->where('customer.status','1')->count();
            ...
            $output = '<ul class="dropdown-menu" style="display:block; position:relative">';
                foreach($data as $row)
                {
                    $output .= '<li class="custname" style="margin-left:1rem;" id='.$row->id.'>'.$row->name.'-'.$row->phone.'-'.$row->documentnumber.'</li>';
                }
                    $output .= '</ul>';            
            } 
            else if($count1>0){
    ...
    {
        if($request->get('documentnumber'))
        {
            $query = $request->get('documentnumber');      
            // fetching result from single table starts here
            ...
            $data = Customerdocuments::where('documentnumber', 'LIKE', "%{$query}%")
                ...
                foreach($data as $row)
                {
                    $output .= '<li class="custname" style="margin-left:1rem;" id='.$row->cust_id.'>'.$row->documentnumber.'</li>';
                }
                    $output .= '</ul>';            
            } else {
                $output = '';
    ...
    public function viewdocdetails(Request $request)
    { 
        $id = $request->id;
        ...
        $test = Customerdocuments::leftJoin('legaldocuments', 'legaldocuments.id','=','customerdocuments.documenttype_id')
                    ->join('registration', 'customerdocuments.cust_id','=','registration.cust_id')
                    ->join('customer','customerdocuments.cust_id','=','customer.id')
                    ->where('customerdocuments.id','=', $id)
                    ->select('customerdocuments.*','legaldocuments.documenttype','registration.id as reg_id','customer.id as user_id')
                    ->first();
    ...
    public function docedit(Request $request){
        ...
        //dd($request);
        $custid = $request->input('hiddenid');
        $id = $request->input('testid');
        $userid = $request->input('userid');
        $documents = Customerdocuments::find($custid);   
        if ($request->hasFile('docS')) {         
        $file = $request->file('docS'); 
        $documentName = 'custdoc_'.time().'.'.$file->extension();  
        ...
        $documentexpirydate = $request->input('docexpirydate');
        $tempcustid = $request->input('token');
        // $file = $request->file('docattachement_1');
        ...
        ->select('legaldocuments.documenttype','customerdocuments.documentnumber','customerdocuments.documentexpirydate','customerdocuments.documentexpirydate','customerdocuments.documentfilename','customerdocuments.id as docid')
        ->get();
            $data['custdocuments'] = $custdocuments;
            $data['success'] = 1;
            // dd("haii");
        return response()->json($data);
        //return redirect("edit-registration/".$id);
        
    }
    
    public function docregedit(Request $request){
        
        
        $custid = $request->input('hiddenid');
        $id = $request->input('registr_id');
        $userid = $request->input('userid');
        $documents = Customerdocuments::find($custid);   
        if ($request->hasFile('docS')) {         
        $file = $request->file('docS'); 
