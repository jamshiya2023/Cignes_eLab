...
<style>
            ...
            <div class="container-fluid">
                <div class="row">
                                ...
                                <div class="col-xl-12">
                                    <div class="card">
                                        <div class="card-header">
                                            <h6 class="card-title m-0">Test Category</h6><a class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#add-sample-type">Add Test Category</a>
                                        ...
                                        <div class="card-body">
                                            ...
                                            <table id="testcategorynew" class="table card-table table-hover align-middle mb-0" style="width: 100%;">
                                            
                                                <thead>
                                                    <tr>
                                                        <th style="width:3%">#</th>
                                                        <th style="width:40%">Test Category</th>
                                                        <th style="width:40%">Test Category (Arabic)</th>
                                                        <th style="width:10%" class="text-center">Ordering</th>
                                                ...
                                                @foreach($testcategory as $key => $value)
                                                    <tr >
                                                        <td>{{ ++$key }}</td>
                                                        <td>{{ $value->testcategory }}</td>
                                                        <td>
                                                            @php if($value->testcategory_arabic) { $arabicname = $value->testcategory_arabic; } else { $arabicname = '-NA-'; } @endphp    
                                                        {{ $arabicname }}</td>
                                                        <td class="text-center">
                                                        <!--<select class="form-select form-select-lg orderingselect" onChange="orderingvalidate(this.value, {{$value->id}});" >
                                                            @foreach($ordering as $order)
                                                            <option value="{{ $order->ordering }}" {{ ( $order->ordering == $value->ordering) ? 'selected' : '' }}>{{ $order->ordering }}</option>
                                                            @endforeach
                                                        </select>-->
                                                        <select class="form-select form-select-lg orderingselect" onChange="orderingvalidate(this.value, {{$value->id}});">
                                                           
                                                            @for ($i = 1; $i <= $testcategoryCount; $i++)
                                                                <option value="{{ $i }}" {{ ($i == $value->ordering) ? 'selected' : '' }}>{{ $i }}</option>
                                                            @endfor
                                                        </select>
                                                        </td>
                                                        <td class="text-center">
                                                            ...
                                                            <a href="{{ 'block-test-category/'.$value->id }}"  onclick="return confirm('Do you want to block this test category?')" class="btn btn-link btn-sm text-info" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="Active" aria-label="Active">
                                                                <i class="fa fa-ban" style="color:#198754;"></i>                                                                
                                                            </a>
                                                            <?php } else { ?>
                                                                <a href="{{ 'unblock-test-category/'.$value->id }}" onclick="return confirm('Do you want to unblock this test category?')" class="btn btn-link btn-sm text-danger" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="Blocked" aria-label="Blocked">
                    ...
                    @endforeach
                            ...
                            <form class="row g-3" id="addtestcategory" name="addtestcategory" method="post" action="{{ route('testcategory.add') }}">                            
                                @csrf
                                <div class="modal fade" id="add-sample-type" tabindex="-1" aria-hidden="true">
                                    <div class="modal-dialog modal-lg modal-dialog-scrollable">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title">Test Category</h5>
                                                <button type="button" id="add-modal-close-x" class="btn-close"></button>
                                            </div>
                                            <div class="modal-body custom_scroll">
                                                <div class="row g-4">
                                                    <div class="col-sm-6">
                                                        <label class="form-label">Test Category Name <span class="red">*</span></label>
                                                        <input type="text" class="form-control form-control-lg" placeholder="Enter here" name="testcategoryname" id="testcategoryname" >
                                                        <div class="alert-danger pt-1 pb-1 px-1 py-1" id="testcategorynamealert" style="display:none ;">Please enter test category </div>                                                
                                                    </div>
                                                    
                                                    <div class="col-sm-6 text-end">
                                                        <label class="form-label">Test Category Name (Arabic)<span class="red">*</span></label>
                                                        <input type="text" dir="rtl" class="form-control form-control-lg" name="arabictestcategoryname" id="arabictestcategoryname" >
                                                    <div class="alert-danger pt-1 pb-1 px-1 py-1" id="testcategorynamealertar" style="display:none ;">Please enter test category </div>                                                

                                                    </div>
                                                </div>                                
                                            </div>
                                            <div class="modal-footer">
                                                <a class="btn btn-primary" onclick="addtestcategoryValidation();">Save</a>
                            ...
                            <form class="row g-3" name="edittestcategory" method="post" action="{{ route('testcategory.edit') }}">                            
                                @csrf
                                        ...
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body custom_scroll">
                                                <div class="row g-4">
                                                    <div class="col-sm-6">
                                                        <label class="form-label">Test Category Name <span class="red">*</span></label>
                                                        <input type="text" class="form-control form-control-lg" name="testcategorynameedit" id="testcategorynameedit" >
                                                        <div class="alert-danger pt-1 pb-1 px-1 py-1" id="testcategorynamealertedit" style="display:none ;">Please enter test category </div>                                                
                                                    </div>
                                                    <div class="col-sm-6 text-end">
                                                        <label class="form-label">Test Category Name (Arabic)<span class="red">*</span></label>
                                                        <input type="text"  dir="rtl" class="form-control form-control-lg" name="arabictestcategorynameedit" id="arabictestcategorynameedit" >
                                                    <div class="alert-danger pt-1 pb-1 px-1 py-1" id="testcategorynamealerteditar" style="display:none ;">Please enter test category </div>                                                
                                            ...
                                            <input type="hidden" class="form-control form-control-lg" name="hiddenid" id="hiddenid"  >
                                                <a class="btn btn-primary" onclick="edittestcategoryValidation();">Save</a>
    ...
    $(document).ready(function () {
        ...
        function orderingvalidate(currentid, id) {
            //alert('Old id-> '+id);
          //  alert('Current id-> '+currentid);
            var token = $('input[name="_token"]').val();

            $.ajax({
            url:"{{ 'test-category-order-update' }}",
            type: 'POST',
            data: { 
                id:id,
                cid:currentid,
                ...
                window.location.href = "{{url('/test-category')}}";
            }
            
        });

        }
    /* $('.orderingselect').change(function(id) {
    ...
    $('#add-modal-close-x, #add-modal-close').click(function() {
        ...
        $('#arabictestcategoryname').val('');
        $('#testcategorynamealert').hide();
        $('#add-sample-type').modal('hide');
    });

    $('#edit-modal-close-x, #edit-modal-close').click(function() {
        $('#testcategorynamealertedit').hide();
        $('#edit-sample-type').modal('hide');
    });

   
    function addtestcategoryValidation() {
        document.getElementById('testcategorynamealert').style.display = 'none';
        document.getElementById('testcategorynamealertar').style.display = 'none';
        if(document.addtestcategory.testcategoryname.value == ''){
            document.getElementById('testcategorynamealert').style.display = 'block';  
            document.addtestcategory.testcategoryname.focus(); 
            return false;     
        }
        if(document.addtestcategory.arabictestcategoryname.value == ''){
            document.getElementById('testcategorynamealertar').style.display = 'block';  
            document.addtestcategory.arabictestcategoryname.focus(); 
            return false;     
        }
    document.addtestcategory.submit();
    return true;
    }
    ...
    function editview(id){
        ...
        $.ajax({
            ...
            success: function(response){
                $('#testcategorynameedit').val(response.testcategory);
                $('#arabictestcategorynameedit').val(response.testcategory_arabic);
    ...
    function edittestcategoryValidation() {
        if(document.edittestcategory.testcategorynameedit.value == ''){
            document.getElementById('testcategorynamealertedit').style.display = 'block';  
            document.edittestcategory.testcategorynameedit.focus(); 
            return false;     
        }
         if(document.edittestcategory.arabictestcategorynameedit.value == ''){
            document.getElementById('testcategorynamealerteditar').style.display = 'block';  
            document.edittestcategory.arabictestcategorynameedit.focus(); 
            return false;     
        }
        
        
    document.edittestcategory.submit();
    return true;
    }
    </script>
