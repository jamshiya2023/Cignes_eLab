...
class SampleCollectionsController extends Controller
{
         ...
         $rejectreason=RejectReason::all();
            $sampledata = DB::table('registration')
               ...
               // ->orWhere('testresult.status','pending')
                ...
                ->get();
                $sampledata = $sampledata->sortByDesc('sort');
              //  dd($sampledata);
               return view('samplecollection', ['rejectreason' => $rejectreason,'sampledata' => $sampledata,'frmdate'=>'','todate'=>'','customer'=>'','registerno'=>'','phone'=>'']);
            }
    ...
    public function searchsamplecollection(Request $request)
    {
        ...
        if($phone){
            ...
            $sampledata = $sampledata->where('customer.phone', 'like', "%$phone%");
        ...
        $sampledata = $sampledata->groupBy('registration.id')->get();
         $rejectreason=RejectReason::all();
               return view('samplecollection', ['rejectreason' => $rejectreason,'sampledata' => $sampledata,'frmdate'=>$frmdate,'todate'=>$todate,'customer'=>$customer,'registerno'=>$registerno,'phone'=>$phone]);
    ...
    public function samplecollectionupdate(Request $request){
        ...
        ->select('testresult.id as id', 'testresult.status as status', 'master_reject_reason.rejectreason as rejectionreason', 'testresult.reg_id as rid', 'alltests.testname as testname','testresult.sample_received_date_time as received_date','testresult.sample_collected_date_time as collection_date', 'testresult.sample_collection_note as samplecollectionnote')
            ->where(function($query){
                $query->where('testresult.status','=','pending')
                ->orWhere('testresult.status','=','notcollected')
                ->orWhere('testresult.status','=','rejected')
                ->orWhere('testresult.status','=','partially collected')
                ->orWhere('testresult.status','=','collected');
         })
...
//dd($request->input('cnote'));
    ...
    public function updateDatabase(Request $request)
    {
        ...
        $collection_notes = $request->input('collection_note');
        $collection_dates = $request->input('collection_date');
        $recev_dates = $request->input('recev_date');
        
        // Loop through the ids array and update each row in the database
        foreach ($ids as $index => $id) {
            $note = $collection_notes[$index];
            $date = $collection_dates[$index];
            $r_date = $recev_dates[$index];
          //dd($value);
            $upquery = Testresult::where('id', $id)
                ->update([
                    'status' => $value,
                    'sample_collection_note' => $note,
                    'sample_collected_date_time' => $date,
                    'sample_received_date_time' =>$r_date,
           ...
           if (!$upquery) {
             ...
             $status = $value;
                ...
                $testdetails = Testresult::where('id',$id)->select('*')->first();

               /* $testreport = new TestReport;
                $testreport->reg_id = $testdetails->reg_id;
               // $testreport->invoice_id = '1';
                $testreport->test_id = $testdetails->test_id;
                $testreport->test_sample_id = $testdetails->id;
                $testreport->status = '1';
                $testreport->save();*/
    ...
    public function updateDatabaseStatus(Request $request)
    { 
        $value = $request->input('status');
        $id = $request->input('id');
        $collection_notes = $request->input('collected_note');
        $collection_dates = $request->input('collected_date');
        $recev_dates = $request->input('recv_date');
        
        $upquery = Testresult::where('id', $id)
                ->update([
                    'status' => $value,
                    'sample_collection_note' => $collection_notes,
                    'sample_collected_date_time' => $collection_dates,
                    'sample_received_date_time' =>$recev_dates,
                    'updated_at' => date("Y-m-d h:i:s"),
                ]);
          $testdetails = Testresult::where('id',$id)->select('*')->first();      
    // dd($testdetails->reg_id);
        
        return response()->json(['success' => true,'reg_id'=>$testdetails->reg_id]);
    }
