                                ...
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body custom_scroll">
                                        <div class="col-md-12 text-end">
                                            <button class="btn btn-primary" style="width:19%;" onclick="updateAllStatus()">Accept All</button>
                                        </div>
...
function sampleupdates(id){ //alert(id);
          ...
          $.each(response.reasons, function( index, reason ) {
            ...
            $.each(response.testlists, function( index, tests ) {
               ...
               var style ='';
                ...
                if(teststatus == 'accepted'){ statushtml = '<span class="btn btn-sm bg-success text-white" style="padding: .10rem .5rem; width:100px;">Accepted</span>';} 
                if(teststatus == 'collected')
                {
                    style += '<span class="btn btn-sm bg-warning text-dark" style="padding: .10rem .5rem; width:120px;">Pending</span>';
                }
                else
                {
                    style += '<span class="btn btn-sm bg-success text-white" style="padding: .10rem .5rem; width:100px;">Accepted</span>';
                }
                testlist += '<tr><td style="width:3%">'+num+'</td><td style="width:10%">'+tests.testname+'</td><td class="text-center" style="width:10%">'+statushtml+'</td><td class="text-center" style="width:10%">'+monthname+' '+day+', '+year+' '+ftime+' '+timeformat+'</td><td class="text-center" style="width:10%">'+rmonthname+' '+rday+', '+ryear+' '+rtime+' '+rtimeformat+'</td><td style="width:20%">'+samplecollectionnote+'</td><td style="width:20%;text-align:center;"><button class="statusToggle status-toggle_ack" data-id="'+tests.id+'" onclick="statusToggle(this);" data-status="'+teststatus+'">'+style+'</button></td></tr>';
            });
            
            $('#headid').html(response.rid);
            $('#samplealert').html('');
            if(response.testlistsCount > 0)
            {
            ...
            {
                testlist +='<tr><td colspan="7">Collected Sample List is Empty</td>'
                $('#tblcollectionpopup').html(testlist);
            }
...
function updateAllStatus() {
    // Iterate over each status toggle button
    $('.status-toggle_ack').each(function() {
        var id = $(this).data('id');
        var currentStatus = $(this).data('status');
        var newStatus = (currentStatus === 'collected') ? 'accepted' : 'collected';

        // Update the button text
        $(this).html('<span class="btn btn-sm ' + getStatusButtonClass(newStatus) + '">' + getStatusButtonText(newStatus) + '</span>');

        // Perform the AJAX request to update the status in the database
        $.ajax({
            url: "{{ route('update-acknow-status') }}",
            type: 'POST',
            data: {
                id: id,
                status: newStatus,
