        ...
        <div class="page-body px-xl-4 px-sm-2 px-0 py-lg-2 py-1 mt-3">
            <div class="container-fluid"> 
                ...
                <!-- SINGLE machine STARTS HERE -->
                    <div class="col-xl-12">
                        ...
                        <div class="card">
                            ...
                            <div class="card-header">
                                <h6 class="card-title m-0">Machines</h6> <a class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#add-machine">Add Machine</a>
                            ...
                            @if(\Session::get('success'))
                                ...
                                @endif
                            <div class="card-body">
                                <table id="tblmachinenew" class="table card-table table-hover align-middle mb-0" style="width: 100%;">
                                    <thead>
                                        <tr>
                                            <th style="width:5%">#</th>
                                            <th style="width:35%">Machine</th>
                                            <th style="width:35%">Machine (Arabic)</th>
                                            <th style="width:10%">Machine Serial No</th>
                                    ...
                                    @php if($machine->machine_serial_no != '') {
                                        $machineno = $machine->machine_serial_no;
                                    } else { 
                                        $machineno ="-NA-";
                                    }
                                    @endphp
                                        <tr>
                                            ...
                                            <td>
                                                @php if($machine->machine_name_arabic) { $arabicname = $machine->machine_name_arabic; } else { $arabicname = '-NA-'; } @endphp
                                                {{$arabicname}}
                                            </td>
                                            <td>{{$machineno}}</td>
                                           
                                            <td class="text-center">
                                            <a  onclick="editview('{{$machine->id }}');"class="btn btn-link btn-sm text-secondry" data-bs-toggle="modal" data-bs-target="#edit-machine{{ $machine->id }}"> 
                                                    <i class="fa fa-edit" style="color:#555CB8;"></i>
                                                </a>
                                                    
                                             @php if($machine->status == '1') { @endphp
                                                <a href="{{ 'block-machine/'.$machine->id }}" onclick="return confirm('Do you want to block this machine?')" class="btn btn-link btn-sm text-info" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="Active" aria-label="Active">
                                                <i class="fa fa-ban" style="color:#198754;"></i>
                                             </a>

                                             @php } else { @endphp
                                                <a href="{{ 'unblock-machine/'.$machine->id }}" onclick="return confirm('Do you want to unblock this machine?')" class="btn btn-link btn-sm text-danger" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="Blocked" aria-label="Blocked">
                    ...
                    <!-- SINGLE machine LISTING ENDS HERE -->
  <!-- ADD machine POPUP STARTS HERE -->
                                   
                 <form class="row g-3" id="addmachine" name="addmachine" method="post" action="{{ route('machine.add') }}">                              
                @csrf 
                        <div class="modal fade" id="add-machine" tabindex="-1" aria-hidden="true">
                            <div class="modal-dialog modal-lg modal-dialog-scrollable">
                                <div class="modal-content">
                                    ...
                                    <div class="row g-4">
                                    <div class="col-sm-6">
                                        <label class="form-label">Machine Name <span class="red">*</span></label>                                        
                                        <input type="text" class="form-control form-control-lg" name="machinename" id="machinename" >  
                                        <div class="alert-danger pt-1 pb-1 px-1 py-1" id="machinenamealert" style="display:none ;">Please enter machine name</div>                                      
                                    </div>

                                    <div class="col-sm-6 text-end">
                                        <label class="form-label">Machine Name (Arabic)<span class="red">*</span></label>                                        
                                        <input dir="rtl" type="text" class="form-control form-control-lg" name="arabicmachinename" id="arabicmachinename" >  
                                        <div class="alert-danger pt-1 pb-1 px-1 py-1" id="machinenamealertar" style="display:none ;">Please enter machine name</div> 
                                    </div>
                                    ...
                                    <div class="col-sm-12">
                                        ...
                                        <input type="text" class="form-control form-control-lg" name="machineno" id="machineno" >  
...
<!-- EDIT machine POPUP STARTS HERE -->
<form class="row g-3" id="editmachine" name="editmachine"  method="post" action="{{ route('machine.edit') }}" >                             
                @csrf 
               
   
                    <div class="modal fade" id="edit-machine-type" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog modal-lg modal-dialog-scrollable">
                            <div class="modal-content">
                                <div class="modal-header">
                                ...
                                <div class="modal-body custom_scroll">
                                    <div class="row g-4">
                                        <div class="col-sm-6">
                                            <label class="form-label">Machine Name <span class="red">*</span></label> 
                                            <input type="text" class="form-control form-control-lg" id="machinenameedit" name="machinenameedit" >                                            
                                            <div class="alert-danger pt-1 pb-1 px-1 py-1" id="machinenameeditalert" style="display:none ;">Please enter machine name</div>                                        
                                        </div>
                                        <div class="col-sm-6">
                                            <label class="form-label text-end">Machine Name (Arabic)<span class="red">*</span></label> 
                                            <input type="text" dir="rtl" class="form-control form-control-lg" id="arabicmachinenameedit" name="arabicmachinenameedit" >
                                        <div class="alert-danger pt-1 pb-1 px-1 py-1" id="machinenameeditalertar" style="display:none ;">Please enter machine name</div>                                        

                                        </div>
                                        <div class="col-sm-12">
                                        <label class="form-label">Machine Serial No</label>                                        
                                        <input type="text" class="form-control form-control-lg" name="machinenoedit" id="machinenoedit" >  
                                ...
                                <ul class="list-group list-group-flush list-group-custom custom_scroll mb-0" style="height: 300px;">
                                    ...
                                    <li class="list-group-item py-3">
                                        <div class="d-flex align-items-center">
                                            ...
                                            <div class="flex-fill ms-3 text-truncate">
                                                <p class="mb-0 color-800">machine Photography</p>
...
<script type="text/javascript">
    $(document).ready(function () {
        $('#add-machine,#edit-machine-type').modal({
            backdrop: 'static',
            keyboard: false
        })
   });
    $('#edit-modal-close-x,#edit-modal-close').click(function(){
        $('#machinenamealertedit').hide();
        $('#edit-machine-type').modal('hide');
    });
        // ADD machine FORM VALIDATION STARTS HERE
    function formValidation()
    {
    document.getElementById('machinenamealert').style.display = 'none';
     document.getElementById('machinenamealertar').style.display = 'none';
     if(document.addmachine.machinename.value == ''){
        document.getElementById('machinenamealert').style.display = 'block'; 
        document.addmachine.machinename.focus();  
        return false;
     }
      if(document.addmachine.arabicmachinename.value == ''){
        document.getElementById('machinenamealertar').style.display = 'block'; 
        document.addmachine.arabicmachinename.focus();  
        return false;
     }
     document.addmachine.submit();    
return true;
...
success:function(response){
    $('#machinenameedit').val(response.machine_name);
    $('#arabicmachinenameedit').val(response.machine_name_arabic);
    $('#machinenoedit').val(response.machine_serial_no);
    $('#hiddenid').val(response.id);
    $('#edit-machine-type').modal('show');
    }
});
...
function formeditValidation()
{
    ...
    document.getElementById('machinenameeditalert').style.display = 'none';
     document.getElementById('machinenameeditalertar').style.display = 'none';
     if(document.editmachine.machinenameedit.value == ''){
        document.getElementById('machinenameeditalert').style.display = 'block'; 
        document.editmachine.machinenameedit.focus();  
        return false;
     }
     if(document.editmachine.arabicmachinenameedit.value == ''){
        document.getElementById('machinenameeditalertar').style.display = 'block'; 
        document.editmachine.arabicmachinenameedit.focus();  
        return false;
     }
     document.editmachine.submit();    
return true;
};
