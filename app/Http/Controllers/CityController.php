...
class CityController extends Controller
{
    public function cityview()
    {
        if(Auth::check()){
            ...
            ->select('country.country_name as countryname','state.state_name as statename', 'city.id as id', 'city.city_name as cityname','city.status as status', 'city.city_namear as citynamear')
            ->orderBy('id','asc')
            ->get();
    ...
    public function cityadd(Request $request)
    {
        if(Auth::check()){
            ...
            $city->city_namear = $request->input('citynamear');
            $city->status = '1';
            $city->save();
    ...
    {         
       ...
       $city = City::join('country','country.id','=','city.country_id')
                ->join('state','state.id','=','city.state_id')
                ->where('city.id',$id)
                ->select('city.id as cityid','city.country_id as countryid','city.state_id as stateid','city.city_name','city.city_namear')
                ->first();
    ...
    { 
        ...
        $city->city_name = $request->input('citynameedit');
         $city->city_namear = $request->input('citynameeditar');
        $city->update();
