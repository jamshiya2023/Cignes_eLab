    ...
    {       if(Auth::check()){        
                $sampledata = Registration::join('customer','registration.cust_id','=','customer.id')
                                    ...
                                    //->join('staff','staff.id','=','testresult.staff_id')
                                     ->join('sample_status_details','sample_status_details.sample_id','=','testresult.id')
                                    ->select('customer.name as customername', 'customer.phone as customerphone', 'customer.place as customerplace', 'registration.id as regid', 'registration.registerdate as regdate', 'registration.registertime as regtime','sample_status_details.created_at as samplerej')
                                    ...
                                    ->get();
                                    //->toSql();
 $sampledata = $sampledata->sortByDesc('samplerej');
                                 //   dd($sampledata);    
