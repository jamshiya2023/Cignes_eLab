...
<style>
    ...
    #tblcontainer.table.card-table tr td, #tblcontainer.table.card-table tr th {
        ...
        <div class="page-body px-xl-4 px-sm-2 px-0 py-lg-2 py-1 mt-3">
            <div class="container-fluid"> 
                ...
                <!-- SINGLE container STARTS HERE -->
                    <div class="col-xl-12">
                        ...
                        <div class="card">
                            ...
                            <div class="card-header">
                                <h6 class="card-title m-0">Containers</h6> <a class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#add-container">Add Container</a>
                            ...
                            @if(\Session::get('success'))
                                ...
                                @endif
                            <div class="card-body">
                                <table id="tblcontainer" class="table card-table table-hover align-middle mb-0" style="width: 100%;">
                                    <thead>
                                        <tr>
                                            <th style="width:5%">#</th>
                                            <th style="width:30%">Container</th>
                                            <th style="width:30%">Container (Arabic)</th>
                                            <th style="width:20%">Container Code</th>
                                    ...
                                    @php if($container->container_serial_no != '') {
                                        $containerno = $container->container_serial_no;
                                    } else { 
                                        $containerno ="-NA-";
                                    }
                                    @endphp
                                        <tr>
                                            ...
                                            <td>
                                                @php 
                                                if($container->container_name_arabic) { $arabicname = $container->container_name_arabic; } else { $arabicname = '-NA-'; }
                                                @endphp
                                                {{$arabicname}}
                                            </td>
                                            <td>{{$containerno}}</td>
                                            ...
                                            <a  onclick="editview('{{$container->id }}');"class="btn btn-link btn-sm text-secondry" data-bs-toggle="modal" data-bs-target="#edit-container{{ $container->id }}"> 
                                                    <i class="fa fa-edit" style="color:#555CB8;"></i>
                                                </a>
                                                    
                                             @php if($container->status == '1') { @endphp
                                                <a href="{{ 'block-container/'.$container->id }}" onclick="return confirm('Do you want to block this container?')" class="btn btn-link btn-sm text-info" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="Active" aria-label="Active">
                                                <i class="fa fa-ban" style="color:#198754;"></i>
                                             </a>

                                             @php } else { @endphp
                                                <a href="{{ 'unblock-container/'.$container->id }}" onclick="return confirm('Do you want to unblock this container?')" class="btn btn-link btn-sm text-danger" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="Blocked" aria-label="Blocked">
                    ...
                    <!-- SINGLE container LISTING ENDS HERE -->
  <!-- ADD container POPUP STARTS HERE -->
                                   
                 <form class="row g-3" id="addcontainer" name="addcontainer" method="post" action="{{ route('container.add') }}">                              
                @csrf 
                        <div class="modal fade" id="add-container" tabindex="-1" aria-hidden="true">
                            <div class="modal-dialog modal-lg modal-dialog-scrollable">
                                <div class="modal-content">
                                    ...
                                    <div class="row g-4">
                                    <div class="col-sm-6">
                                        <label class="form-label">Container Name <span class="red">*</span></label>                                        
                                        <input type="text" class="form-control form-control-lg" name="containername" id="containername" >  
                                        <div class="alert-danger pt-1 pb-1 px-1 py-1" id="containernamealert" style="display:none ;">Please enter container name</div>                                      
                                    </div>

                                    <div class="col-sm-6 text-end">
                                        <label class="form-label">Container Name(Arabic) <span class="red">*</span></label>  
                                        <input type="text" dir="rtl" class="form-control form-control-lg" name="arabiccontainername" id="arabiccontainername" >
                                    <div class="alert-danger pt-1 pb-1 px-1 py-1" id="containernamealertar" style="display:none ;">Please enter container name</div>                                      

                                    </div>

                                    <div class="col-sm-12">
                                        <label class="form-label">Container Code</label>                                        
                                        <input type="text" class="form-control form-control-lg" name="containerno" id="containerno" >  
...
<!-- EDIT container POPUP STARTS HERE -->
<form class="row g-3" id="editcontainer" name="editcontainer"  method="post" action="{{ route('container.edit') }}" >                             
                @csrf 
               
   
                    <div class="modal fade" id="edit-container-type" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog modal-lg modal-dialog-scrollable">
                            <div class="modal-content">
                                <div class="modal-header">
                                ...
                                <div class="modal-body custom_scroll">
                                    <div class="row g-4">
                                        <div class="col-sm-6">
                                            <label class="form-label">Container Name <span class="red">*</span></label> 
                                            <input type="text" class="form-control form-control-lg" id="containernameedit" name="containernameedit" >                                            
                                            <div class="alert-danger pt-1 pb-1 px-1 py-1" id="containernameeditalert" style="display:none ;">Please enter container name</div>                                        
                                        </div>
                                        <div class="col-sm-6 text-end">
                                            <label class="form-label">Container Name(Arabic) <span class="red">*</span></label>  
                                            <input type="text" dir="rtl" class="form-control form-control-lg" name="arabiccontainernameedit" id="arabiccontainernameedit" > 
                                        <div class="alert-danger pt-1 pb-1 px-1 py-1" id="containernameeditalertar" style="display:none ;">Please enter container name</div>                                        

                                        </div>
                                        <div class="col-sm-12">
                                        <label class="form-label">Container Code</label>                                        
                                        <input type="text" class="form-control form-control-lg" name="containernoedit" id="containernoedit" >  
                                ...
                                <ul class="list-group list-group-flush list-group-custom custom_scroll mb-0" style="height: 300px;">
                                    ...
                                    <li class="list-group-item py-3">
                                        <div class="d-flex align-items-center">
                                            ...
                                            <div class="flex-fill ms-3 text-truncate">
                                                <p class="mb-0 color-800">container Photography</p>
...
<script type="text/javascript">
    $(document).ready(function () {
        $('#add-container,#edit-container-type').modal({
            backdrop: 'static',
            keyboard: false
        })
   });
    $('#edit-modal-close-x,#edit-modal-close').click(function(){
        $('#containernamealertedit').hide();
        $('#edit-container-type').modal('hide');
    });
        // ADD container FORM VALIDATION STARTS HERE
    function formValidation()
    {
    document.getElementById('containernamealert').style.display = 'none';
    document.getElementById('containernamealertar').style.display = 'none';
     if(document.addcontainer.containername.value == ''){
        document.getElementById('containernamealert').style.display = 'block'; 
        document.addcontainer.containername.focus();  
        return false;
     }
     if(document.addcontainer.arabiccontainername.value == ''){
        document.getElementById('containernamealertar').style.display = 'block'; 
        document.addcontainer.arabiccontainername.focus();  
        return false;
     }
     document.addcontainer.submit();    
return true;
...
success:function(response){
    $('#containernameedit').val(response.container_name);
    $('#arabiccontainernameedit').val(response.container_name_arabic);
    $('#containernoedit').val(response.container_serial_no);
    $('#hiddenid').val(response.id);
    $('#edit-container-type').modal('show');
    }
});
...
{    
    document.getElementById('containernameeditalert').style.display = 'none';
    document.getElementById('containernameeditalertar').style.display = 'none';
     if(document.editcontainer.containernameedit.value == ''){
        document.getElementById('containernameeditalert').style.display = 'block'; 
        document.editcontainer.containernameedit.focus();  
        return false;
     }
     if(document.editcontainer.arabiccontainernameedit.value == ''){
        document.getElementById('containernameeditalertar').style.display = 'block'; 
        document.editcontainer.arabiccontainernameedit.focus();  
        return false;
     }
     document.editcontainer.submit();    
return true;
};
...
$(document).ready(function() {
  $('#tblcontainer').DataTable({
    ...
    "lengthMenu": [10, 25, 50, 75, 100], // Define the available page lengths
    "pageLength": 10, // Set the initial page length
    "ordering": true, // Enable table sorting
    "order": [], // Disable initial sorting (let DataTables handle it)
  });
});
</script>
