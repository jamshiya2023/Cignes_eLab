...
$(document).on('click', '.status-toggle', function() { 
    ...
    // Toggle the status
    var newStatus = (currentStatus === 'pending') ? 'Accepted' : 'pending';
                    ...
                    success: function(response){
                        ...
                        $.each(response.tests, function( index, test ) {
                            ...
                            monthname = new Date(collecteddate).toLocaleString('en-us',{month:'short'})
                                ...
                                if(teststatus == 'collected'){ statushtml = '<span class="btn btn-sm bg-success text-white" style="padding: .10rem .5rem; width:100px;">Collected</span>';} 
                               testlist += '<tr><td style="width:3%">'+num+'</td><td style="width:10%">'+test.testname+'</td><td class="text-center">'+statushtml+'</td><td class="text-center" style="width:10%">'+monthname+' '+test.collectiondate+'</td><td class="text-center" style="width:10%">'+test.recevieddate+'</td><td style="width:20%">'+samplecollectionnote+'</td><td style="width:20%"><button class="status-toggle" data-id="'+tests.id+'" data-status="pending">pending</button></td></tr>';
                            });
