...
class PaymentMethodController extends Controller
{
    public function paymentmethodview()
    {
        if(Auth::check()){
            $userId = Auth::id();
            $paymentmethods = PaymentMethod::select('id','paymentmethod','status','paymentmethodar')
            ->orderBy('id','desc')
            ->get();
    ...
    public function paymentmethodadd(Request $request)
    {
        if(Auth::check()){
            ...
            $paymentmethod->paymentmethodar = $request->input('paymentmethodnamear');
    ...
    {         
       ...
       $paymentmethod = PaymentMethod::where('id',$id)
                ->select('id','paymentmethod','paymentmethodar')
                ->first();
    ...
    public function paymentmethodedit(Request $request)
{
    ...
    if (!$paymentmethod) {
        return redirect("payment-method")->withError('Payment method not found');
    }
    
    $paymentmethod->paymentmethod = $request->input('paymentmethodnameedit');
    $paymentmethod->paymentmethodar = $request->input('paymentmethodnameeditar');
    $paymentmethod->save();
