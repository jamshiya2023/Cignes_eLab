...
class StateController extends Controller
{
    public function stateview()
    {
        if(Auth::check()){
            ...
            ->get();
            $state = State::join('country','country.id','=','state.country_id')->select('state.id as id','state.state_name','state.state_namear','state.status as status','country.country_name as countryname')
            ->orderBy('id','asc')
            ->get();
    ...
    public function stateadd(Request $request)
    {
        if(Auth::check()){
            ...
            $state->state_name = $request->input('statename');
             $state->state_namear = $request->input('statenamear');
            $state->status = '1';
            $state->save();
    ...
    {         
       ...
       $state = State::where('id',$id)
                ->select('id as sid','country_id','state_name','state_namear')
                ->get();
    ...
    { 
        ...
        $country->state_namear = $request->input('statenameeditar');
        $country->update();
